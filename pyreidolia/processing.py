...
def resize(img: Union[np.ndarray, np.array], img_size: Tuple[int, int] = (350, 525)):
...
def dilate(img: Union[np.array, np.ndarray], ksize: Tuple[int, int] = (7, 7)):
    """dilate an image
...
def draw_convex_hull(mask: Union[np.array, np.ndarray], mode: str = "convex"):
...
def post_process(
    probability: np.array, threshold: Union[float, int], min_size: Union[float, int]
):
