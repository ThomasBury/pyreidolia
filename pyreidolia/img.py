...
def pd_get_resolution_sharpness(
    df: pd.DataFrame, train_path: str, test_path: str, im_id_col: str = "image_id"
) -> pd.DataFrame:
...
def get_resolution_sharpness(im: np.array, train_path: str, test_path: str):
    """Compute the resolution and sharpness
    ...
    im : np.array
        the image
    train_path : str
        the path pointing to the training images
    test_path : str
        the path pointing to the training images
    ...
    -------
    pd.Series
        the series with the resolution and sharpness
    """
    if os.path.exists(train_path + im):
        img = cv2.imread(train_path + im, cv2.IMREAD_UNCHANGED)
    else:
